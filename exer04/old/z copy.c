...
int main(int argc, char *argv[])
{
	...
	int submatrix_size = n / NUM_CLIENTS;
	int **submatrices[NUM_CLIENTS];
	for (int i = 0; i < NUM_CLIENTS; i++)
	{
		submatrices[i] = (int **)malloc(submatrix_size * sizeof(int *));
		for (int j = 0; j < submatrix_size; j++)
		{
			submatrices[i][j] = (int *)malloc(submatrix_size * sizeof(int));
			for (int k = 0; k < submatrix_size; k++)
			{
				submatrices[i][j][k] = M[i * submatrix_size + k][j * submatrix_size + k];
			}
		}
	}
	...
	while (TRUE)
	{
		...
		if (FD_ISSET(master_socket, &readfds))
		{
			...
			// // Send the matrix data to the client
			// for (int row = 0; row < n; row++)
			// {
			// 	for (int col = 0; col < n; col++)
			// 	{
			// 		char numStr[20]; // Assuming 20 is enough for the string representation of an int
			// 		sprintf(numStr, "%d ", M[row][col]);
			// 		send(new_socket, numStr, strlen(numStr), 0);
			// 	}
			// 	char newline = '\n';
			// 	send(new_socket, &newline, 1, 0); // Send newline character to separate rows
			// }

			// puts("Matrix sent successfully to the client");
			...
			if (client_index != -1)
			{
				// Send the assigned submatrix to the client
				for (int row = 0; row < submatrix_size; row++)
				{
					for (int col = 0; col < submatrix_size; col++)
					{
						char numStr[20]; // Assuming 20 is enough for the string representation of an int
						sprintf(numStr, "%d ", submatrices[client_index][row][col]);
						send(new_socket, numStr, strlen(numStr), 0);
					}
					char newline = '\n';
					send(new_socket, &newline, 1, 0); // Send newline character to separate rows
